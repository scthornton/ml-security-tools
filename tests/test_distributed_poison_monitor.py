...
class TestGradientSnapshotSerialisation:
    def test_to_json_produces_valid_json(self):
        snap = dpm.GradientSnapshot(
            rank=0, step=1, global_l2=1.5, global_linf=0.8, mean=0.01, std=0.5, parameter_count=100
        )
    ...
    def test_round_trip_through_json(self):
        snap = dpm.GradientSnapshot(
            rank=2, step=5, global_l2=2.3, global_linf=1.1, mean=-0.05, std=0.9, parameter_count=512
        )
    ...
    def test_sendto_called_on_record(self, tmp_dir):
        ...
        with patch("socket.socket") as mock_socket_cls:
            ...
            snapshotter = dpm.GradientSnapshotter(
                model, rank=0, log_dir=str(tmp_dir), broadcast_endpoint="127.0.0.1:5454"
            )
            snapshotter.record(step=1)
...
class TestLoadLogs:
    def test_loads_single_worker_file(self, tmp_dir):
        snaps = [
            dpm.GradientSnapshot(rank=0, step=i, global_l2=1.0, global_linf=0.5, mean=0.0, std=1.0, parameter_count=100)
            for i in range(3)
        ]
    ...
    def test_loads_multiple_worker_files(self, tmp_dir):
        for rank in range(3):
            snaps = [
                dpm.GradientSnapshot(
                    rank=rank, step=0, global_l2=1.0, global_linf=0.5, mean=0.0, std=1.0, parameter_count=100
                )
            ]
    ...
    def test_skips_blank_lines(self, tmp_dir):
        ...
        snap = dpm.GradientSnapshot(
            rank=0, step=1, global_l2=1.0, global_linf=0.5, mean=0.0, std=1.0, parameter_count=100
        )
        path.write_text("\n" + snap.to_json() + "\n\n")
        loaded = dpm.load_logs(tmp_dir)
        assert len(loaded) == 1
...
class TestComputeMetrics:
    def test_compute_step_metrics_correct_l2_ratio(self):
        snaps = [
            ...
            dpm.GradientSnapshot(
                rank=1, step=0, global_l2=4.0, global_linf=2.0, mean=0.0, std=1.0, parameter_count=100
            ),
        ]
    ...
    def test_compute_divergence_skips_single_worker_step(self):
        snaps = [
            dpm.GradientSnapshot(rank=0, step=1, global_l2=1.0, global_linf=0.5, mean=0.0, std=1.0, parameter_count=100)
        ]
    ...
    def test_compute_divergence_includes_multi_worker_step(self):
        snaps = [
            dpm.GradientSnapshot(
                rank=0, step=2, global_l2=1.0, global_linf=0.5, mean=0.0, std=1.0, parameter_count=100
            ),
            dpm.GradientSnapshot(
                rank=1, step=2, global_l2=2.0, global_linf=1.0, mean=0.0, std=1.0, parameter_count=100
            ),
        ]
        result = dpm.compute_divergence(snaps)
        assert 2 in result
    ...
    def test_mean_spread_computed_correctly(self):
        snaps = [
            dpm.GradientSnapshot(
                rank=0, step=0, global_l2=1.0, global_linf=0.5, mean=0.1, std=1.0, parameter_count=100
            ),
            dpm.GradientSnapshot(
                rank=1, step=0, global_l2=1.0, global_linf=0.5, mean=0.5, std=1.0, parameter_count=100
            ),
        ]
        metrics = dpm.compute_step_metrics(snaps)
...
class TestDetectChangepoints:
    def test_returns_empty_on_stable_divergences(self):
        ...
        drift = 1.1
        divergences = {i: {"l2_ratio": drift, "linf_ratio": 1.0, "mean_spread": 0.01, "workers": 2} for i in range(20)}
    ...
    def test_steps_are_in_sorted_order(self):
        divergences = {
            3: {"l2_ratio": 1.0, "linf_ratio": 1.0, "mean_spread": 0.0, "workers": 2},
